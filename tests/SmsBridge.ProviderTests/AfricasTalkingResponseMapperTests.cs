...
public sealed class AfricasTalkingResponseMapperTests
{
    ...
    public void FromResponse_MapsAcceptedStatusesAsSuccessful(
        int statusCode,
        SmsDeliveryStatus expectedStatus)
    {
        ...
        result.ProviderMessageId.Should().Be("ATPid_123");
    }
        ...
        bool expectedTransient)
    {
        ...
        result.Status.Should().Be(expectedStatus);
        result.ErrorCode.Should().Be(statusCode.ToString(CultureInfo.InvariantCulture));
        result.ErrorMessage.Should().Be("ProviderStatus");
        result.IsTransientFailure.Should().Be(expectedTransient);
    }

    [Fact]
    public void FromResponse_ReturnsFailureForInvalidJson()
    {
        var result = AfricasTalkingSmsResponseMapper.FromResponse("africas-talking", "{");

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Be("Unable to parse Africa's Talking response.");
    }
